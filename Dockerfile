...
CMD bash -c "monofy bugsink-show-version && bugsink-manage check --deploy --fail-level WARNING && bugsink-manage migrate && bugsink-manage prestart && gunicorn --config bugsink/gunicorn.docker.conf.py --bind=0.0.0.0:$PORT --access-logfile - bugsink.wsgi || bugsink-runsnappea"
